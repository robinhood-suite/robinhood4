...
struct rbh_filter *
parse_expression(struct filters_context *ctx, int argc, char **argv,
                 int *arg_idx, const struct rbh_filter *_filter,
                 ...
                 parse_callback cb, void *cb_param)
{
    ...
    for (i = *arg_idx; i < argc; i++) {
        ...
        token = str2command_line_token(ctx, argv[i], &pe_index);
        switch (token) {
        case CLT_URI:
        ...
        case CLT_OR:
            ...
            tmp = parse_expression(ctx, argc, argv, &i, &negated_left_filter,
        ...
        case CLT_PARENTHESIS_OPEN:
            ...
            tmp = parse_expression(ctx, argc, argv, &i, &left_filter, sorts,
        ...
        default:
            if (cb)
                cb(argc, argv, &i, &left_filter, sorts, sorts_count, token,
                   cb_param);
            break;
        }
