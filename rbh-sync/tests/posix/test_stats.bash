...
test_mpi_stats()
{
    ...
    rbh_sync rbh:posix-mpi:. rbh:$db:$testdb --stats
    ...
    echo "$output" | grep "progress"
    echo "$output" | grep "progress" | cut -d':' -f2 | cut -d' ' -f2
    local entry_count="$(echo "$output" | grep "progress" | cut -d':' -f2 |
                         cut -d' ' -f2)"

    echo "entry_count = '$entry_count'"
    # There should be 14 numbers in $entry_count, sum them all into one
    entry_count="$(echo "$entry_count" | paste -s -d+ - | bc)"
    echo "entry_count after = '$entry_count'"
