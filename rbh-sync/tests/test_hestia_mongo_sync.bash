...
hestia_get_attr()
{
    ...
    echo "$(hestia_obj read "$obj" | grep "$attr" | head -n 1 |
...
test_user_attributes()
{
    ...
    hestia_obj put_data $obj2 --file /etc/hosts
#--id_fmt=parent_id
    hestia metadata update --id_fmt=parent_id --input_fmt=key_value $obj1 <<< data.keys=alicia
    hestia metadata update --id_fmt=parent_id --input_fmt=key_value $obj1 <<< data.blob=blob

    hestia_obj move_data $obj2 --source 0 --target 1
    hestia metadata update --id_fmt=parent_id --input_fmt=key_value $obj2 <<< data.mic=test

    hestia object read $obj

    rbh_sync "rbh:hestia:" "rbh:mongo:$testdb"

    find_attribute '"xattrs.user_metadata.keys":"alicia"' '"ns.name":"'$obj1'"'
