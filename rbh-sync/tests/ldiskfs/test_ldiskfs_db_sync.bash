...
rbh_sync_ldiskfs()
{
    rbh_sync "rbh:ldiskfs:/dev/mapper/mds1_flakey" "$1"
}
...
test_simple_sync()
{
    ...
    rbh_sync_ldiskfs "rbh:$db:$testdb"

    find_attribute '"ns.xattrs.path":"'$testdir_mds'/'$testdb'/"'
    find_attribute '"ns.xattrs.path":"'$testdir_mds'/'$testdb'/dir/"'
    find_attribute '"ns.xattrs.path":"'$testdir_mds'/'$testdb'/dir/fileA"'
    find_attribute '"ns.xattrs.path":"'$testdir_mds'/'$testdb'/dir/fileB"'
}
...
#
#     rbh_sync_ldiskfs "$first_dir#$second_dir" "rbh:$db:$testdb"
#
...
#
#     rbh_sync_ldiskfs "$abs_path#$second_dir" "rbh:$db:$testdb"
#
...
#     do_db drop $testdb
#
#     rbh_sync_ldiskfs "$first_dir#$second_dir/$third_dir" "rbh:$db:$testdb"
#
#     ! (find_attribute '"ns.name":"'$first_dir'"')
#     ! (find_attribute '"ns.name":"'$second_dir'"')
#
...
#     # The migrate enforce test_gen_1 to have a gen of 1
#     lfs migrate test_gen_1
#     lfs setstripe -E 1k -c 2 -E -1 -c -1 "test_gen_2"
#     truncate -s 2k test_gen_2
#     lfs setstripe -E 1M -c 2 -S 256k -E -1 -c -1 "test_gen_3"
...
#         # with these values
#         mdt_indexes=$(lfs getdirstripe 'test_mdt_index2' |
#                       tail -n +3 | awk '{print $1}' | xargs | tr ' ' ',')
...
# test_mdt_hash()
# {
#     # Make sure to create directories on a different MDT for the migration.
...
#     # FIXME the actual issue is that we should add all_char as the default
#     # when no LMV is found on the directory or treat no mdt_hash in the DB as
#     # being equivalent to all_char when filtering. Though lfs find doesn't
#     # output a directory with default striping with the filter -H all_char.
#
...
lfs setdirstripe -D -i 0 -c 1 $LUSTRE_DIR
tmpdir=$(mktemp --directory --tmpdir=$LUSTRE_DIR)
trap -- "rm -rf '$tmpdir'" EXIT
cd "$tmpdir"
testdir_mds=$(sed -r 's/^\/\w+\/\w+(\/\w+.*)/\1/g' <<< "$tmpdir")
