...
test_stop_sync_on_error(){
    ...
    if [[ $db_count -lt 1 ]]; then
        error "Invalid number of files were synced, expected atleast '1' 
    ...
    # with the second file and fail, or  start with the first file
    ...
    (echo $first_file_att | grep "No entry found"  && echo $second_file_att \
            | grep "No entry found" && echo $third_file_att | grep "No entry found") \
            || (find_attribute '"ns.name":"'$first_file'"' && echo $second_file_att | \
            grep "No entry found" && echo $third_file_att | grep "No entry found") \
