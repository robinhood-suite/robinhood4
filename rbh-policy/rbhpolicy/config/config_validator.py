...
def validate_policy(name, condition, action, trigger, parameters=None,
    ...
    if not (stoptrigger is None or isinstance(stoptrigger, BaseTrigger)):
        raise TypeError(f"Policy stoptrigger must be a BaseTrigger instance or "
                        f"None, got {type(stoptrigger).__name__}")
