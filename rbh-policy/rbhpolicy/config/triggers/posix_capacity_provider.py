...
def _extract_posix_path(database_uri: Optional[str]) -> Optional[str]:
    if not database_uri:
        return None

    source_backend = resolve_backend_type(database_uri)
    if source_backend != "posix":
        return None

    mountpoint = resolve_backend_mountpoint(database_uri)
    if mountpoint and os.path.isabs(mountpoint):
        return mountpoint

    return None

def _resolve_posix_provider(database_uri: Optional[str]) -> Optional[CapacityProvider]:
    path = _extract_posix_path(database_uri)
    if path is None:
        return None
