...
def normalize_policy_sort(sort):
    ...
    Accepted shapes:
      - None
      - {"sort_by": "atime", "sort_order": "asc"}

    Returns:
      - None if no sort requested
      - dict with backend field name and order:
...
def cmd(cmdline: str) -> str:
    ...
    Named placeholders like ``{param_name}`` are resolved later against the
...
def normalize_action(action):
    ...
    - If action is a string without ':' → common:<string>
    - Otherwise raises TypeError or ValueError.
    - Lambda functions are forbidden.
    """
    if callable(action):
        ...
        if name == "<lambda>":
            raise ValueError(
                "Lambda functions cannot be used as actions. "
    ...
    if isinstance(action, str):
        ...
        raise ValueError(
            f"Unsupported action format '{action}'."
        )
