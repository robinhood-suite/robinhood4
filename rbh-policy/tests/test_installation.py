...
class TestRBHPolicySystemInstall(unittest.TestCase):
    ...
    def test_package_directory_exists(self):
        """Check that the 'rbhpolicy' directory exists in site-packages"""
        if not self.package_installed:
            self.skipTest("Package directory not found — skip install check")
        self.assertTrue(os.path.isdir(self.package_path),
                        f"Directory '{self.package_path}' not found")
