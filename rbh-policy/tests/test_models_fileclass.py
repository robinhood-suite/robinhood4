...
# Patch ctypes.CDLL so that when code tries to load "librobinhood.so", it is
# redirected to the one in BUILD_LIB_PATH instead of the system path. This
# allows tests to run against the freshly built library without installing it.
...
class TestFileClassBehavior(unittest.TestCase):
    ...
    def test_declare_and_validate_fileclass(self):
        BigFiles = declare_fileclass(name="BigFiles",
                target=entities.Size >= "1000MB")
        self._validate_class(BigFiles)

    def test_combined_fileclass_and_condition(self):
        AdminsFiles = declare_fileclass(name="AdminsFiles",
                target=entities.User == "root")
