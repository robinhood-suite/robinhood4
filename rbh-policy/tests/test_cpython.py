...
import rbhpolicy.config.filter
rbhpolicy.config.filter.backend = "rbh:lustre:test"
...
class TestFilter(unittest.TestCase):
    def test_string_field_filter(self):
        ...
        self.assertIsNotNone(f, "[ERROR] build_filter returned NULL for Type == 'file'")
    ...
    def test_logical_and_different_fields(self):
        ...
        self.assertIsNotNone(combined, "[ERROR] rbh_filter_and returned NULL for UID & GID")
    ...
    def test_logical_or_different_fields(self):
        ...
        self.assertIsNotNone(combined, "[ERROR] rbh_filter_or returned NULL for Type OR Type")
        self.assertEqual(rbh_filter_validate(combined), 0)
    ...
    def test_uid_string_value_should_fail(self):
        ...
        f = build_filter(["-uid", "not_a_number"])
        self.assertIsNone(f, "[ERROR] build_filter should return None for UID == 'not_a_number'")

    def test_unknown_field_should_fail(self):
        """Unknown field name should result in validation failure"""
        print("[TEST] Creating filter: -unknown 123")
        with self.assertRaises(ValueError):
            build_filter(["-unknown", "123"])

    def test_unknown_field_should_fail(self):
        """Unknown field name should result in validation failure"""
        print("[TEST] Creating filter: -unknown 123")
        f = build_filter(["-unknown", "123"])
        self.assertIsNone(f, "[ERROR] build_filter should return None for unknown predicates")
