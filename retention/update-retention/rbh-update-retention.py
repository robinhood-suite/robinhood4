...
def handle_non_relative_expiration(context, directory):
    print(f"Directory '{directory.path}' expired on '{directory.expiration_date}'")
...
def check_directory_expirancy(context, _dir_info):
    ...
    if not dir_info[0]:
        print(f"Directory with ID '{dir_info[3]}' doesn't have a proper path, "
              "skipping it.")
        return
    elif dir_info[1] is "None":
        print(f"Expiration attribute is unknown, so we cannot determine if the "
              "directory is really expired or not. Skipping '{directory.path}'")
