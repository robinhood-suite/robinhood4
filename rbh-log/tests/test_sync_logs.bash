...
---" > $conf
    ...
    rbh_log rbh:$db:$testdb --sync 5 | grep "Command" | cut -d':' -f2- |
        ...
        difflines "rbh-sync --config conf rbh:posix:file rbh:mongo:test_sync_logs-test_command_line --no-skip --one" \
                  "rbh-sync --config conf rbh:posix:. rbh:mongo:test_sync_logs-test_command_line --no-skip" \
                  "rbh-sync --config conf rbh:posix:. rbh:mongo:test_sync_logs-test_command_line" \
                  "rbh-sync rbh:posix:dir rbh:mongo:test_sync_logs-test_command_line" \
                  "rbh-sync rbh:posix:. rbh:mongo:test_sync_logs-test_command_line"
}
