...
---" > $conf
    ...
    rbh_log rbh:$db:$testdb --find 6 | grep "Command" | cut -d':' -f2- |
        ...
        difflines "rbh-find --config conf rbh:mongo:test_find_logs-test_command_line --alias blob -count" \
                  "rbh-find --config conf rbh:mongo:test_find_logs-test_command_line -size +3 -uid 30 -printf blob\n" \
                  "rbh-find --config conf --verbose rbh:mongo:test_find_logs-test_command_line -size +3" \
                  "rbh-find --config conf rbh:mongo:test_find_logs-test_command_line" \
