...
list can be checked with rbh-info(1). These plugins act as ways to interact with
a storage system, either by fetching metadata from it, or by updating data in
it.
...
For ID targeting, here is another example for a Lustre ID in a Mongo database:
