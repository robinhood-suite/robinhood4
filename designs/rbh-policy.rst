...
The trigger evaluation behavior differs depending on the execution context:
...
- Manual execution of one or more policies.
...
policy, and if the trigger is satisfied, executes the associated rules and actions.
...
.. code:: bash
   ...
   rbh-policy --daemon -c <config_file> <policy1>[,<policy2>,...] [options]

Arguments:
- ``--daemon``: Enables daemon mode.
- ``-c <config_file>``: Path to the configuration file.
- ``<policy1>,<policy2>,...``: One or more policy names to monitor.
- ``all``: Special keyword to monitor all policies defined in the file.

Available options:
- ``--verbose``: Enables detailed output during daemon execution,
                 including matched entries, rules, and executed actions.
...
.. code:: bash
   ...
   # Daemon with detailed output for troubleshooting
