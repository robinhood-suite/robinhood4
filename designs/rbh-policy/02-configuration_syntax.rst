...
2. Global Evaluation Interval Declaration: ``evaluation_interval = "<duration>"``
3. Fileclasses Declaration: ``declare_fileclass``
4. Policy Declaration: ``declare_policy``
...
metrics-based triggers using the current state of the MongoDB database.
...
- ``evaluation_interval``: How frequently the policy engine evaluates
  metrics-based triggers against the current database state.

Notes:
- This parameter is required and must appear exactly once in the configuration.
- The engine does not control metadata synchronization.
