...
The subdirectories discovered when retrieving the children of an entry can be
stored to avoid querying the backend each time after updating all the children.
With this approach, we only need to query entries without a path at the start of
the update process. If other directories not in all the current trees are
renamed during the update process, they will be detected in the next update
cycle.
