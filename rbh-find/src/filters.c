...
struct rbh_filter *
regex2filter(const struct rbh_filter_field *field, const char *shell_regex,
...
shell_regex2filter(enum predicate predicate, const char *shell_regex,
                   unsigned int regex_options)
{
    return regex2filter(&predicate2filter_field[predicate], shell_regex,
                        regex_options);
}
...
size2filter(const char *_size)
{
    ...
    if (size == 0ULL)
        error(EX_USAGE, 0,
              "size arguments to should start with at least one digit");
