...
ctx_finish(struct find_context *ctx)
{
    ...
    free(ctx->backends);
    for (size_t i = 0; i < ctx->info_plugin_count; i++)
        rbh_backend_plugin_destroy(ctx->info_plugins[i]->plugin.name);
                       ...
                       int *plugin_index)
{
    ...
    case '-':
        ...
        for (int i = 0; i < ctx->info_plugin_count; ++i) {
            const struct rbh_backend_plugin *info_plugin = ctx->info_plugins[i];
            enum rbh_parser_token token =
                info_plugin->ops->check_valid_token(string);
    ...
    for (i = *arg_idx; i < ctx->argc; i++) {
        ...
        case CLT_SORT:
            ...
            *sorts = sort_options_append(*sorts, (*sorts_count)++,
            ...
            break;
        case CLT_PREDICATE:
            assert(plugin_index != -1);
            /* Build a filter from the predicate and its arguments */
            const struct rbh_backend_plugin *info_plugin =
                ctx->info_plugins[plugin_index];

            tmp = info_plugin->ops->build_filter((const char **) ctx->argv,
                                                 ctx->argc, &i,
                                                 &ctx->need_prefetch);
            if (negate) {
                tmp = rbh_filter_not(tmp);
