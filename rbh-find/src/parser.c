                    ...
                    enum command_line_token token, void *param)
{
    ...
    switch (token) {
    case CLT_OUTPUT_MODIFIER:
        switch (str2output_modifier(ctx->argv[*arg_idx])) {
        ...
        case OUTPUT_MODIFIER_SORT:
            {
                ...
                /* Build an options filter from the sort option and
                 * its arguments */
        ...
        case OUTPUT_MODIFIER_LIMIT:
            {
                ...
                if (str2uint64_t(ctx->argv[*arg_idx], &options->limit) != 0)
                    error(EX_USAGE, 0, "invalid limit '%s'",
                            ctx->argv[*arg_idx]);
            }
            break;
