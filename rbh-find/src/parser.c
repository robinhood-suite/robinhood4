...
{
    struct find_context *ctx = (struct find_context *)param;
    bool ascending = true;
    ...
    case CLT_SORT:
        ...
        if (*arg_idx + 1 >= argc)
            error(EX_USAGE, 0, "missing argument to '%s'", argv[*arg_idx]);
        *sorts = sort_options_append(*sorts, (*sorts_count)++,
                                     str2field(argv[++(*arg_idx)]), ascending);
        break;
    case CLT_ACTION:
        ctx->action_done = true;
        find(ctx, str2action(argv[*arg_idx]), arg_idx, filter, *sorts,
             *sorts_count);
        break;
    default:
        break;
