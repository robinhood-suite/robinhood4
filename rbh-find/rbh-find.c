...
usage(const char *backend)
{
    const char *message =
        ...
        "                         print information about each entry according\n"
        "                         to the specified FORMAT_STRING. Native POSIX\n"
        "                         directives are written as is (i.e. 'size' is\n"
        "                         '%%s' but other plugin and extension directives\n"
        "                         should be written with a starting 'R', then\n"
        "                         the plugin/extension specific directive category\n"
        "                         and finally the directive itself. For instance,\n"
        "                         '%%RLf' for the Lustre FID. There are also some\n"
        "                         directives available accross all backends\n"
        "               %%d       Depth\n"
        "               %%f       Name\n"
        "               %%h       Directory name (if applicable)\n"
        "               %%H       Backend\n"
        "               %%I       ID in base 64\n"
        "               %%p       path\n"
...
main(int _argc, char *_argv[])
{
    ...
    if (command_context.print_stats)
        rbh_print_log(&metadata, RBH_FIND_LOG, NULL);
