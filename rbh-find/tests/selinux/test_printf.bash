...
{
    local context="user_u:object_r:user_tmp_t:s0:c0,c3-s3:c0.c3,c1000"
    local ls_out
    ...
    rbh_find "rbh:$db:$testdb" -name file1 \
        -printf "%RZu:%RZr:%RZt:%RZR\n" |
        difflines "$context"

    ls_out=$(ls -Z file1 | sed 's# file1$# /file1#')

    rbh_find "rbh:$db:$testdb" -name file1 -printf "%RZc %p\n" |
        difflines "$ls_out"
}
