...
test_invalid()
{
    rbh_find "rbh:mongo:$testdb" -expired blob &&
        error "find with -expired shouldn't have any argument"

    rbh_find "rbh:mongo:$testdb" -expired-at "" &&
        error "find with an invalid expired-at epoch should have failed"

    rbh_find "rbh:mongo:$testdb" -expired-at $(echo 2^64 | bc) &&
        error "find with an expired-at epoch too big should have failed"

    rbh_find "rbh:mongo:$testdb" -expired-at 42blob &&
        error "find with an invalid expired-at epoch should have failed"

    rbh_find "rbh:mongo:$testdb" -expired-at invalid &&
        error "find with an invalid expired-at epoch should have failed"
...
test_expired_abs()
{
    ...
    rbh_find "rbh:mongo:$testdb" -expired-at $(date +%s) | sort |
        difflines "/$fileA" "/$fileC"
    rbh_find "rbh:mongo:$testdb" -expired | sort |
        difflines "/$fileA" "/$fileC"
    rbh_find "rbh:mongo:$testdb" -expired-at $timeA | sort |
        difflines "/$fileA" "/$fileC"
    rbh_find "rbh:mongo:$testdb" -expired-at +$timeA | sort |
        difflines "/$fileB" "/$fileD"
    rbh_find "rbh:mongo:$testdb" -expired-at -$timeD | sort |
        difflines "/$fileA" "/$fileB" "/$fileC"
    rbh_find "rbh:mongo:$testdb" -expired-at +$timeD | sort |
        difflines
    rbh_find "rbh:mongo:$testdb" -expired-at -$timeC | sort |
        difflines
    rbh_find "rbh:mongo:$testdb" -expired-at 18446744073709551615 | sort |
        difflines "/$fileA" "/$fileB" "/$fileC" "/$fileD"
    rbh_find "rbh:mongo:$testdb" -expired-at +18446744073709551614 | sort |
        difflines
    rbh_find "rbh:mongo:$testdb" -expired-at inf | sort |
        difflines "/$fileE"
}
...
test_expired_rel()
{
    ...
    rbh_find "rbh:mongo:$testdb" -expired-at -$(date +%s --date='1 hour') |
...
test_printf_expiration_info()
{
    ...
    rbh_find "rbh:mongo:$testdb" -printf "%p %E\n" | sort |
...
---" > $conf_file
    ...
    rbh_sync --config $conf_file "rbh:retention:$dir" "rbh:mongo:$testdb"

    rbh_find --config $conf_file "rbh:mongo:$testdb" -printf "%p %e\n" | sort |
