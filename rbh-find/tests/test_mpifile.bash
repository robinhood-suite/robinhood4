...
test_not()
{
    ...
    rbh_find "rbh:mpi-file:../$testdb.mfu" "(" -not -size 1k ")" -a \
        "(" -name "1*" ")" | sort | difflines "/1M"
}
...
test_or()
{
    ...
    rbh_find "rbh:mpi-file:../$testdb.mfu" "(" -type f -a -size +1k ")" -o \
        "(" -type d -a -size 1k ")" | sort | difflines "/" "/1M"
