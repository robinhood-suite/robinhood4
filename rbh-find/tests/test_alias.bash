...
test_config_specified() {
    echo -e "alias: \n  a1: \"-perm 001 -name file -printf \'%A\'\"" \
            > test_conf.yaml
    rbh-find --config test_conf.yaml "rbh:mongo:$testdb" --alias a1
    if [ $? -ne 0 ]; then
...
test_config_default() {
    CONFIG_FILE="/etc/robinhood4/rbh_conf.yaml"
    ...
    rbh-find "rbh:mongo:$testdb" --alias a1
    if [ $? -ne 0 ]; then
...
test_multiple_alias() {
    echo -e "alias: \n  a1: \"-perm 001 -name file -printf \'%A\'\"\n  a2: \
            \"-name file -printf \'%A\'\"" > test_conf.yaml

    rbh-find --config test_conf.yaml "rbh:mongo:$testdb" --alias a2 --alias \
        a1 --alias a2
    if [ $? -ne 0 ]; then
