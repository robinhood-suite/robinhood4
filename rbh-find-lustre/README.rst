...
default stripe count will be outputted.
...
rbh-lfind defines a ``-pattern`` predicate that filters entries based on the
pattern of any of their components. The only accepted values are ``default``,
``raid0`` and ``mdt``. It behaves exactly like the ``stripe-count`` predicate
otherwise.
