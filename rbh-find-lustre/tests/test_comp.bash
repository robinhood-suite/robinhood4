...
test_comp_start()
{
    ...
    rbh_lfind "rbh:mongo:$testdb" -comp-start 1M -comp-start 512M | sort |
        difflines "/$file"
    rbh_lfind "rbh:mongo:$testdb" -comp-start 1M,512M | sort |
        difflines "/$file"
    rbh_lfind "rbh:mongo:$testdb" -comp-start 1M -comp-start 511M | sort |
        difflines
    rbh_lfind "rbh:mongo:$testdb" -comp-start 1M,511M | sort |
        difflines "/$file"
...
test_comp_end()
{
    ...
    lfs setstripe -E 1M -c 1 -S 256k -E 512M -c 2 -S 512k -E -1 -c -1 -S 1024k \
        "$file"
    truncate -s 1M "$file"
#"end" : [ NumberLong(1048576), NumberLong(536870912), NumberLong(-1) ]
    rbh_sync "rbh:lustre:." "rbh:mongo:$testdb"

    lfs getstripe -v "$file"
    mongo $testdb --eval "db.entries.find()"

    rbh_lfind "rbh:mongo:$testdb" -comp-end -1k | sort |
        difflines
    rbh_lfind "rbh:mongo:$testdb" -comp-end +1k | sort |
        difflines "/$file"
    rbh_lfind "rbh:mongo:$testdb" -comp-end 1023k -or -comp-end 1k | sort |
        difflines
    rbh_lfind "rbh:mongo:$testdb" -comp-end -2T | sort |
        difflines "/$file"
    rbh_lfind "rbh:mongo:$testdb" -comp-end 1M -comp-end 512M | sort |
        difflines "/$file"
    rbh_lfind "rbh:mongo:$testdb" -comp-end 1M,512M | sort |
        difflines "/$file"
    rbh_lfind "rbh:mongo:$testdb" -comp-end 1M -comp-end 511M | sort |
        difflines
    rbh_lfind "rbh:mongo:$testdb" -comp-end 1M,511M | sort |
        difflines "/$file"
    rbh_lfind "rbh:mongo:$testdb" -not '(' -comp-end 1M -comp-end 511M ')' |
        sort | difflines "/" "/$file"
}
