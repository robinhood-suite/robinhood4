...
test_retention_after_sync()
{
    ...
    local expiration_date="$(( $(stat -c %X $dir) + 10))"

    rbh-sync rbh:lustre:. "rbh:mongo:$testdb"

    find_attribute '"xattrs.user.expiration_date":'$expiration_date \
                   '"ns.xattrs.path":'/$dir

    date --set="@$(( $(stat -c %X $dir) + 5))"
    ...
    expiration_date="$(( $(stat -c %X $dir/$entry) + 10))"
    find_attribute '"xattrs.user.expiration_date":'$expiration_date \
                   '"ns.xattrs.path":"'/$dir
