...
test_avg_size()
{
    ...
    local size="$(((1024 + 1025 + dir_size) / 3))"

    rbh_report "rbh:mongo:$testdb" --output "avg(statx.size)" |
        difflines "$size"
    ...
    dir_size="$(stat -c %s .)"
    size="$(((1024 + 1025 + size_600M + dir_size) / 4))"
...
test_avg_mtime()
{
    ...
    rbh_report "rbh:mongo:$testdb" --output "avg(statx.mtime.sec)" |
        difflines "$mtime"
    ...
    mtime="$(((root_mtime + first_mtime + second_mtime + third_time) / 4))"
...
test_avg_ino()
{
    ...
    rbh_report "rbh:mongo:$testdb" --output "avg(statx.ino)" |
        difflines "$ino"
}
