...
.. [#] simply put, a storage backend which RobinHood can use to store a
       storage system metadata, and later query.
...
The **SOURCE** can be either an event source or stdin, and the **DESTINATION**
...
rbh-fsevents uses RobinHood URIs_ to identify the backends.

.. _URIs: https://github.com/robinhood-suite/robinhood4/blob/main/librobinhood/doc/internals.rst
...
the ``--enrich`` option to fetch additionnal metadata related to an updated
entry before updating it in the destination backend. The ``--enrich`` option
takes as input a RobinHood URI representing the backend to use for metadata
enrichment. The information retrieved depends on the backend used for
enrichment.
...
destination backend (or sent to enrichment if requested). The size of the batch
can be controlled with the ``--batch-size`` option, which specifies how many
entries can be retained for deduplication at once. By default, the deduplication
is enabled and the batch size is set to 100 entries.

Deduplication helps reduce the load on the mirror backend by merging multiple
similar events on the same entry into a single event, or by ignoring some events
altogether. Some events may cancel each other out; for example, if a file is
created and then deleted shortly afterward. Deduplication will see the CREATE
and UNLINK events and simply drop the event.

However, using a large --batch-size can risk consuming a huge amount of memory,
since all events in a batch are kept in memory before updating the mirror
backend. Also, it increases the time before the backend is updated, as we wait
for the batch to be full.
...
add the ``ack-user`` option to the Lustre source URI and specify the user with
which to do the acknowledgment.
...
enriching and updating the events. The events are distributed amongst workers
based on the hash of their `RobinHood ID`_.
