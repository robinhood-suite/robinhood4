...
int
rbh_fsevent_merge_xattr(struct rbh_fsevent *merged,
                        const struct rbh_fsevent *cached_event,
                        ...
                        struct rbh_sstack *stack)
{
    ...
    merged->id.size = cached_event->id.size;
    merged->id.data = rbh_sstack_push(stack, cached_event->id.data,
                                      cached_event->id.size);
    if (!merged->id.data)
        return -1;

    merged->xattrs.count = cached_event->xattrs.count;
    merged->xattrs.pairs = rbh_sstack_push(
        stack, cached_event->xattrs.pairs,
        merged->xattrs.count * sizeof(cached_event->xattrs.pairs)
        );
    if (!merged->xattrs.count)
        return -1;
