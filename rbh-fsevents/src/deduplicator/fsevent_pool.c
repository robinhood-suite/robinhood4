...
struct rbh_fsevent_pool {
    ...
    /* Function to compute an index from the ID's hash */
    size_t (*id2index)(const struct rbh_id *id, size_t size);
                     ...
                     size_t nb_workers)
{
    ...
    if (!strcmp(source->name, "lustre")) {
        ...
        pool->id2index = hash_lu_id2index;
    } else {
        hash_fn = fsevent_pool_hash_id;
        pool->id2index = hash_id2index;
    }
...
insert_new_fsevents_map(struct rbh_fsevent_node *cached_event)
{
    ...
    value_map_insert_pair(cached_event->copy_data,
                          &cached_event->fsevent.xattrs,
                          &rbh_fsevents_pair);
                                  ...
                                  const struct rbh_value *first_string)
{
    ...
    value_map_insert_pair(cached_event->copy_data,
                          (struct rbh_value_map *)rbh_fsevents,
                          &xattrs_pair);
}
                      ...
                      const struct rbh_value_pair *xattr)
{
    ...
    // XXX we discard const here
    value_map_insert_pair(cached_event->copy_data,
                          (struct rbh_value_map *)rbh_fsevents_map,
                          xattr);
}
...
rbh_fsevent_pool_flush(struct rbh_fsevent_pool *pool)
{
    ...
    while (pool->count > 0) {
        ...
        index = pool->id2index(first_id->id, pool->events_size);
