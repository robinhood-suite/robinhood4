...
{
    if (strcmp(arg, "-") == 0) {
        fsevents_md->source_read = xstrdup("stdin");
        /* SOURCE is '-' (stdin) */
        return source_from_file(stdin);
    }

    if (rbh_is_uri(arg))
        return source_from_uri(arg, dump_file, max_changelog, fsevents_md);
...
static void
signal_shutdown(pthread_cond_t *signal)
{
...
consumer_thread(void *arg) {
    ...
    signal_shutdown(cinfo->signal_available_for_work);
