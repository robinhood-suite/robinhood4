...
check_id()
{
    ...
    local update_id=$(echo "$output" | cut -d' ' -f5)
...
check_tier()
{
    ...
    if ! echo "$output" | grep "$tier"; then
        error "'$tier' should be present in '$output'"
    fi

    if ! echo "$output" | grep "$tier" | grep "0,$size"; then
        error "The data on '$tier' should be between 0 and '$size'"
    fi

    entries=$(echo "$output" | grep "$tier" | grep "0,$size")
    entries_lines=$(echo "$entries" | wc -l)
    if [[ $entries_lines -ne 1 ]]; then
...
test_update_data()
{
    ...
    check_tier "${events[0]}" "tier_0" "$(stat -c %s /etc/hosts)"
...
test_update_copy()
{
    ...
    check_tier "${events[0]}" "tier_0" "$(stat -c %s /etc/hosts)"
    check_tier "${events[0]}" "tier_1" "$(stat -c %s /etc/hosts)"
