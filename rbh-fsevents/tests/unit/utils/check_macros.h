...
#define ck_assert_link(X, id, _name, parent)          \
    do {                                              \
        ck_assert_int_eq((X)->type, RBH_FET_LINK);    \
        ck_assert_id_eq(&(X)->id, id);                \
        ck_assert_str_eq((X)->link.name, _name);      \
        ck_assert_id_eq((X)->link.parent_id, parent); \
    } while (0)

#define ck_assert_unlink(X, id, _name, parent)        \
    do {                                              \
        ck_assert_int_eq((X)->type, RBH_FET_UNLINK);  \
        ck_assert_id_eq(&(X)->id, id);                \
        ck_assert_str_eq((X)->link.name, _name);      \
        ck_assert_id_eq((X)->link.parent_id, parent); \
    } while (0)
