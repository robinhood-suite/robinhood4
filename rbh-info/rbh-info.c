...
help()
{
    const char *message =
        ...
        "a given backend\n"
        "  -b --backend_source       Show the backend used as source for past "
        "rbh-syncs\n"
...
_get_backend_source(const struct rbh_value *value)
{
    ...
    for (size_t i = 0; i < value->sequence.count; i++) {
        ...
        for (size_t j = 0; j < submap->count; j++) {
            const struct rbh_value_pair *pair = &submap->pairs[j];

            printf("%s\n", pair->key);
            for (size_t k = 0; k < pair->value->sequence.count; k++)
                printf("%s\n", pair->value->sequence.values[k].string);
        }
    }
}
