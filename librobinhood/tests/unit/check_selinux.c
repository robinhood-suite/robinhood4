...
START_TEST(load_selinux)
{
    const struct rbh_plugin_extension *selinux;
    const struct rbh_backend_plugin *posix;

    posix = rbh_backend_plugin_import("posix");
    ck_assert_ptr_nonnull(posix);

    selinux = rbh_plugin_load_extension(&posix->plugin, "selinux");
    ck_assert_ptr_nonnull(selinux);
    ck_assert_str_eq(posix->plugin.name, selinux->super);
}
END_TEST
