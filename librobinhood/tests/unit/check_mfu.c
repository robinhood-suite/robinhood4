...
START_TEST(load_mfu)
{
    const struct rbh_plugin_extension *mfu;
    const struct rbh_backend_plugin *posix;

    posix = rbh_backend_plugin_import("posix");
    ck_assert_ptr_nonnull(posix);

    mfu = rbh_plugin_load_extension(&posix->plugin, "mfu");
    ck_assert_ptr_nonnull(mfu);
    ck_assert_str_eq(posix->plugin.name, mfu->super);
}
END_TEST
