...
START_TEST(load_acl)
{
    const struct rbh_plugin_extension *acl;
    const struct rbh_backend_plugin *posix;

    posix = rbh_backend_plugin_import("posix");
    ck_assert_ptr_nonnull(posix);

    acl = rbh_plugin_load_extension(&posix->plugin, "acl");
    ck_assert_ptr_nonnull(acl);
    ck_assert_str_eq(posix->plugin.name, acl->super);
}
END_TEST
