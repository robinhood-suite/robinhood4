...
START_TEST(load_sparse)
{
    const struct rbh_plugin_extension *sparse;
    const struct rbh_backend_plugin *posix;

    posix = rbh_backend_plugin_import("posix");
    ck_assert_ptr_nonnull(posix);

    sparse = rbh_plugin_load_extension(&posix->plugin, "sparse");
    ck_assert_ptr_nonnull(sparse);
    ck_assert_str_eq(posix->plugin.name, sparse->super);
}
END_TEST
