...
START_TEST(ria_free)
{
    ...
    for (int i = 0; i < sizeof(LETTERS); i++) {
        STRING[i] = LETTERS[i];
        ck_assert_mem_eq(&STRING[i], &LETTERS[i], sizeof(*STRING));
    }

    letters = rbh_iter_array(STRING, sizeof(*STRING), sizeof(LETTERS), free);
    ck_assert_ptr_nonnull(letters);

    for (size_t i = 0; i < sizeof(LETTERS); i++)
        ck_assert_mem_eq(rbh_iter_next(letters), &LETTERS[i], sizeof(*STRING));
