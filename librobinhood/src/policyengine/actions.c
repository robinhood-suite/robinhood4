...
{
    if (cache->default_action.params.sstack != NULL)
        rbh_sstack_destroy(cache->default_action.params.sstack);

    for (size_t i = 0; i < cache->rule_count; i++) {
        if (cache->rule_actions && cache->rule_actions[i].params.sstack != NULL)
                     ...
                     struct rbh_backend *mi_backend)
{
    ...
    if (action->params.sstack != NULL) {
        const struct rbh_value_map *map = &action->params.map;
