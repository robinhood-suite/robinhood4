                                     ...
                                     const struct filters_context *f_ctx)
{
    ...
    for (size_t i = 0; i < f_ctx->info_pe_count; ++i) {
        ...
        if (ext_matches > 0) {
            fprintf(stderr, "Error: multiple extensions implement action %d. "
    ...
    if (ext_hint != NULL) {
        if (ext_matches == 1 && plugin_hint_matches) {
            fprintf(stderr,
                    ...
                    "implement action %d\n",
                    plugin->plugin.name, action->type);
            errno = EEXIST;
            return NULL;
        }
        ...
        fprintf(stderr,
                "Error: plugin or extension '%s' does not implement action %d\n",
                hint_str, action->type);
        errno = ENOTSUP;
        return NULL;
    }
    ...
    fprintf(stderr, "Error: action %d not supported by backend\n",
            action->type);
    errno = ENOTSUP;
    return NULL;
}
...
static int
rbh_pe_append_log_part(char *buffer, size_t buffer_len, size_t *length,
                       const char *part)
{
    int written;
    ...
    written = snprintf(buffer + *length, buffer_len - *length,
                       *length == 0 ? "%s" : ", %s", part);
    if (written < 0)
        return -1;

    if ((size_t)written >= buffer_len - *length) {
        *length = buffer_len - 1;
        return -1;
    }
...
/**
 * Log an entry by composing log fragments from plugin/extension providers.
 *
                  ...
                  const struct filters_context *f_ctx)
{
    ...
    rbh_pe_action_provider_hint(action, &hint, &hint_len);

    details[0] = '\0';

    /* Compose log output from plugin */
    for (size_t i = 0; i < f_ctx->info_pe_count; ++i) {
        ...
        rc = rbh_pe_common_ops_fill_entry_info(ops, tmp, sizeof(tmp), entry, "L", "");
        if (rc < 0)
            return -1;

        if (rc > 0) {
            tmp[rc < (int)sizeof(tmp) ? rc : (int)sizeof(tmp) - 1] = '\0';
            if (rbh_pe_append_log_part(details, sizeof(details), &details_len, tmp) < 0)
                return -1;

            if (hint == NULL || rbh_pe_name_matches_hint(
                    f_ctx->info_pe[i].plugin->plugin.name, hint, hint_len))
                hint_matched = true;
        }
    ...
    /* Compose log output from extensions */
    for (size_t i = 0; i < f_ctx->info_pe_count; ++i) {
        const struct rbh_plugin_extension *ext;
        const struct rbh_pe_common_operations *ops;
        ...
        if (hint != NULL && !rbh_pe_name_matches_hint(ext->name, hint, hint_len))
            continue;
    ...
    /* Verify hint matched if specified */
    if (hint != NULL && !hint_matched) {
        char hint_str[256];
        size_t copy_len = hint_len < sizeof(hint_str) - 1 ? hint_len : sizeof(hint_str) - 1;
        memcpy(hint_str, hint, copy_len);
        hint_str[copy_len] = '\0';
        fprintf(stderr, "Error: plugin or extension '%s' not found\n", hint_str);
        errno = ENOTSUP;
        return -1;
    }
    ...
    printf("LogAction | %s, params=%s\n", details, params_str ? params_str : "{}");
    free(params_str);
