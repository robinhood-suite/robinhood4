...
// Temporary
const char *rbh_map_find_key(const struct rbh_value_pair *pairs,
                             const char *key) {
    while (pairs != NULL) {
        if (strcmp(pairs->key,key) == 0)
            return pairs->value->string;
        pairs+=1;
    }
    return NULL;
}
                   ...
                   struct rbh_config *config)
{
    ...
    rbh_config_find("RBH_S3", &value, RBH_VT_MAP);
    pairs = value.map.pairs;

    password = rbh_map_find_key(pairs, "S3_PASSWORD");
    address = rbh_map_find_key(pairs, "S3_ADDRESS");
    user = rbh_map_find_key(pairs, "S3_USER");
