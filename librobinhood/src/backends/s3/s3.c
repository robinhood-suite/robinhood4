...
get_next_object(struct s3_iterator *s3_iter)
{
    ...
    size_t *bkt_length = &s3_iter->bkt_data.list_length;
    size_t *curr_obj_id = &s3_iter->obj_data.current_id;
    size_t *obj_length = &s3_iter->obj_data.list_length;
    size_t tmp_object_length;
    char **tmp_object_list;

    if (*curr_obj_id + 1 >= *obj_length) {
        do {
            (*curr_bkt_id)++;
            if (*curr_bkt_id + 1 >= *bkt_length)
                return NULL;

            get_object_list(s3_iter->bkt_data.list[*curr_bkt_id],
                            &tmp_object_length, &tmp_object_list);
        } while (tmp_object_length == 0);

        s3_iter->obj_data.list = malloc(tmp_object_length * sizeof(char *));
        if (s3_iter->obj_data.list == NULL)
            return NULL;

        s3_iter->obj_data.list = tmp_object_list;
        s3_iter->obj_data.list_length = tmp_object_length;
...
s3_iterator_new()
{
    ...
    size_t tmp_bucket_length;
    size_t tmp_object_length;
    char **tmp_bucket_list;
    char **tmp_object_list;
    ...
    get_bucket_list(&tmp_bucket_length, &tmp_bucket_list);
    s3_iter->bkt_data.list_length = tmp_bucket_length;
    s3_iter->bkt_data.list = malloc(tmp_bucket_length * sizeof(char *));
    if (s3_iter->bkt_data.list == NULL)
        return NULL;

    s3_iter->bkt_data.list = tmp_bucket_list;
    s3_iter->bkt_data.current_id = 0;

    get_object_list(s3_iter->bkt_data.list[0], &tmp_object_length,
                    &tmp_object_list);
    s3_iter->obj_data.list_length = tmp_object_length;
