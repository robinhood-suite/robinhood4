...
{
    struct rbh_fsentry *mongo_entry;
    ...
    if (fs_backend->id == RBH_BI_MONGO || fs_backend->id == RBH_BI_SQLITE) {
        ...
        error(EXIT_FAILURE, ENOTSUP,
              ...
              "backend (posix, lustre) instead of metadata-only backend",
              fs_uri, fs_backend->id);
    }

    while (true) {
        errno = 0;
        mongo_entry = rbh_mut_iter_next(mongo_iter);
        ...
        const char *rel_path = rbh_pe_get_path(mongo_entry);
        if (rel_path == NULL) {
        ...
        bool policy_filter_matched = true;
        if (policy->filter != NULL) {
            policy_filter_matched = rbh_filter_matches_fsentry(policy->filter, fresh);
        }

        if (!policy_filter_matched) {
