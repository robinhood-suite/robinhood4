                 ...
                 void *clt_cb_param, void *om_cb_param)
{
    ...
    for (i = *arg_idx; i < ctx->argc; i++) {
        ...
        if (modifier != OUTPUT_MODIFIER_NONE) {
            if (om_cb)
                om_cb(ctx, &i, options, modifier, om_cb_param);
            continue;
        }
        ...
        case CLT_OR:
            ...
            tmp = parse_expression(ctx, &i, &negated_left_filter, options,
                                   clt_cb, om_cb, clt_cb_param, om_cb_param);
        ...
        default:
            if (clt_cb)
                clt_cb(ctx, &i, &left_filter, options, token, clt_cb_param);
            break;
        }
