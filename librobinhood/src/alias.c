                 ...
                 size_t argv_alias_index)
{
    ...
    parse_range = temp_index-alias_index - 1;
    ...
    *argv = argv_temp;
    ...
    do {
        alias_found = false;
        for (int i = alias_index; i < alias_index + parse_range; i++) {
            if ((*argv)[i] && strcmp((*argv)[i], "--alias") == 0) {
                ...
                if (alias_index_for_res != -1) {
                    alias_resolution(argc, argv, alias_index_for_res, i);
                    alias_found = true;
                    break;
                }
            }
