...
s3_get_bucket_list(size_t *number_of_buckets, char ***buckets_list)
{
    ...
    if (outcome.IsSuccess()) {
        ...
        for (size_t i = 0; i < num_bucket; ++i) {
            auto bucket_name = bucket_list[i].GetName();

            bucket_name_list[i] = new char[bucket_name.length() + 1];
            std::strcpy(bucket_name_list[i], bucket_name.c_str());
        }
        *number_of_buckets = num_bucket;
        *buckets_list = bucket_name_list;
    } else {
        std::cerr << "Failed to retrieve bucket list: "
                   ...
                   const char *object_name)
{
    ...
    if (outcome.IsSuccess()) {
        ...
        mtime_ms = results.GetLastModified().Millis();
        mtime_s = std::chrono::duration_cast<std::chrono::seconds>(
                  std::chrono::milliseconds(mtime_ms));
...
{
    static auto mapIterator = s3_metadata_ptr->user_meta.begin();
