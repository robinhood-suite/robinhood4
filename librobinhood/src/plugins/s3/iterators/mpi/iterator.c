...
int get_sub_array_bounds(int array_size, int num_chunks, int chunk_index,
                          int *start, int *end)
{
    ...
    int base_chunk_size = array_size / num_chunks;
    int remainder = array_size % num_chunks;
...
s3_mpi_iterator_new()
{
    ...
    char *sub_string = malloc(64); //Max size of an S3 bucket name
    char **temp_array;
    char **sub_array;
    int arr_start;
    int rcv_size;
    ...
    if (rc == 0) {
        ...
        for (int i = 0; i < rcv_size; i++) {
            sub_array[i] = (char*)malloc(64 * sizeof(char));
            MPI_Recv(sub_string, 64, MPI_CHAR, 0, 0, MPI_COMM_WORLD,
                     MPI_STATUS_IGNORE);
