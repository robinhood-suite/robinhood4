...
static struct rbh_filter *
posix_predicate2filter(struct filters_context *context,
                       const char *predicate, char *argument)
{
    struct filters_context posix_context = *context;
    char *argv[] = {(char *)predicate, argument};
    int index = 0;

    posix_context.argc = 2;
    posix_context.argv = argv;

    for (size_t i = 0; i < context->info_pe_count; i++) {
        const struct rbh_pe_common_operations *common_ops;

        common_ops = get_common_operations(&context->info_pe[i]);

        if (rbh_pe_common_ops_check_valid_token(common_ops, predicate) !=
            RBH_TOKEN_PREDICATE)
            continue;

        return rbh_pe_common_ops_build_filter(common_ops, &posix_context,
                                              &index);
    }

    error(EXIT_FAILURE, 0,
          "no plugin provides predicate '%s'", predicate);
    __builtin_unreachable();
}

static struct rbh_filter *
posix_id2filter(struct filters_context *context,
                const char *predicate, uint32_t id)
{
    char arg[16];

    snprintf(arg, sizeof(arg), "%" PRIu32, id);

    return posix_predicate2filter(context, predicate, arg);
}

static struct rbh_filter *
mode2filter(struct filters_context *context, uint32_t bit)
{
    char arg[16];

    snprintf(arg, sizeof(arg), "-0%" PRIo32, bit);

    return posix_predicate2filter(context, "-perm", arg);
}


static struct rbh_filter *
acl_groups2filter(struct filters_context *context,
                  const struct acl_subject *subject,
                  bool named, int perms)
{
    ...
    for (size_t i = 0; i < subject->gid_count; i++) {
        ...
        if (named) {
            ...
            current = posix_id2filter(context, "-gid", subject->gids[i]);
        }
...
static struct rbh_filter *
owner2filter(struct filters_context *context,
             const struct acl_subject *subject,
             uint32_t owner_bit)
{
    return rbh_filter_and(posix_id2filter(context, "-uid", subject->uid),
                          mode2filter(context, owner_bit));
}
 ...
 */
static struct rbh_filter *
group_match2filter(struct filters_context *context,
                   const struct acl_subject *subject,
                   int perms)
{
    ...
    owning_group = acl_groups2filter(context, subject, false, -1);
    named_groups = acl_groups2filter(context, subject, true, perms);
 ...
 */
static struct rbh_filter *
named_user2filter(struct filters_context *context,
                  const struct acl_subject *subject,
                  uint32_t acl_perm, uint32_t group_bit)
{
    struct rbh_filter *not_owner;
    struct rbh_filter *permissions;

    not_owner = rbh_filter_not(posix_id2filter(context, "-uid", subject->uid));

    permissions = rbh_filter_and(
            acl_named_entry2filter(&predicate2filter_field[APRED_USER],
                                   subject->uid,
                                   acl_perm),
            mode2filter(context, group_bit));
 ...
 */
static struct rbh_filter *
group2filter(struct filters_context *context,
             const struct acl_subject *subject,
             uint32_t acl_perm, uint32_t group_bit)
{
    ...
    more_specific = rbh_filter_not(
            rbh_filter_or(
                posix_id2filter(context, "-uid", subject->uid),
                acl_named_entry2filter(&predicate2filter_field[APRED_USER],
                                       subject->uid,
                                       -1)));

    permissions = rbh_filter_and(
            group_match2filter(context, subject, acl_perm),
            mode2filter(context, group_bit));
 ...
 */
static struct rbh_filter *
other2filter(struct filters_context *context,
             const struct acl_subject *subject,
             uint32_t other_bit)
{
    struct rbh_filter *filter;

    filter = posix_id2filter(context, "-uid", subject->uid);
    filter = rbh_filter_or(
            filter,
            acl_named_entry2filter(
                &predicate2filter_field[APRED_USER],
                subject->uid, -1));
    filter = rbh_filter_or(filter, group_match2filter(context, subject, -1));

    return rbh_filter_and(
            rbh_filter_not(filter),
                mode2filter(context, other_bit));
}


static struct rbh_filter *
acl_access2filter(struct filters_context *context, char *arg, uint32_t acl_perm,
                  uint32_t owner_bit, uint32_t group_bit, uint32_t other_bit)
{
    ...
    filter = owner2filter(context, &subject, owner_bit);

    filter = rbh_filter_or(filter,
            named_user2filter(context, &subject, acl_perm, group_bit));

    filter = rbh_filter_or(filter,
            group2filter(context, &subject, acl_perm, group_bit));

    filter = rbh_filter_or(filter,
            other2filter(context, &subject, other_bit));
...
rbh_acl_build_filter(struct filters_context *context, int *index)
{
    ...
    switch (predicate) {
        ...
        case APRED_DEFAULT_GROUP:
            ...
            break;
        case APRED_READABLE:
            filter = acl_access2filter(context, argv[++i], 4,
                                       S_IRUSR, S_IRGRP, S_IROTH);
            break;
        case APRED_WRITABLE:
            filter = acl_access2filter(context, argv[++i], 2,
                                       S_IWUSR, S_IWGRP, S_IWOTH);
            break;
        case APRED_EXECUTABLE:
            filter = acl_access2filter(context, argv[++i], 1,
