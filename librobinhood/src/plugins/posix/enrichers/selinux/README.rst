 ...
 * For all entries with a ``security.selinux`` extended attribute:
   ...
   * SELinux role
   * SELinux type
   * SELinux range - decomposed in low and high parts, which are also decompsed
     in MLS and MCS parts
...
For instance::
        ...
        system_u:object_r:container_file_t:s0:c1.c7-s1:c123,c456
...
is stored as::
        ...
        selinux.range = s0:c0.c7-s1:c123,c456
        selinux.range.low = s0:c0.c7
        selinux.range.low.sensitivity = s0
        selinux.range.low.categories = c0,c1,c2,c3,c4,c5,c6,c7
        selinux.range.high = s1:c123,c456
        selinux.range.high.sensitivity = s1
        selinux.range.high.categories = c123,c456
