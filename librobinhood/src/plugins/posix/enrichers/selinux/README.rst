...
The `type` field is used by Type Enforcement (TE). For example, a process running
in one domain may only access object types explicitly allowed by the policy.

The last field, the `range` is used by
MLS/MCS mechanisms. In common `targeted` policies, this field is often a simple
...
.. code:: bash
    ...
    rbh-find rbh:mongo:test -selinux-range-readable s0:c123,c456
    ./dir/container-file
...
.. code:: bash
    ...
    rbh-find rbh:mongo:test -selinux-range-writable s0:c123,c456
    ./dir/container-file
