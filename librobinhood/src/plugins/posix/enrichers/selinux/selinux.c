...
static int
parse_prefixed_number(const char **cursor, char prefix, int max)
{
    const char *p = *cursor;
    int value;
...
parse_category_item(const char **cursor, struct selinux_level *level)
{
    ...
    first = parse_prefixed_number(&p, 'c', SELINUX_MAX_CATEGORY);
    if (first < 0)
        return -1;
    ...
    if (*p == '.') {
        ...
        last = parse_prefixed_number(&p, 'c', SELINUX_MAX_CATEGORY);
        if (last < 0)
            return -1;
    }
...
parse_level(const char *string, struct selinux_level *level)
{
    ...
    level->sens = parse_prefixed_number(&p, 's', SELINUX_MAX_SENSITIVITY);
    if (level->sens < 0)
        return -1;
