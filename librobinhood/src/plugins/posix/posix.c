...
static struct rbh_fsentry *
posix_undelete(void *backend, const char *path, struct rbh_fsentry *fsentry)
{
    struct posix_backend *posix = backend;
    int count = 0;
    ...
    while (posix->enrichers && posix->enrichers[count]) {
        if (strcmp(posix->enrichers[count]->extension.name, "lustre") == 0)
            return rbh_pe_common_ops_undelete(
                       posix->enrichers[count]->extension.common_ops, backend,
                       path, fsentry);
        count++;
    }
                 ...
                 char **predicate_helper, char **directive_helper)
{
    ...
    if (type) {
        ...
        if (rbh_posix_backend_load_extensions(plugin, (void *) posix, type))
            goto err;
    }
    ...
    rc = asprintf(predicate_helper,
        ...
        "    -executable USER\n"
        "    -readable   USER\n"
        "    -writeable  USER     filter entries based if they are\n"
        "                         executable, readable or writeable by the user\n"
        "                         provided. The behavior is different from GNU's\n"
        "                         find as it will retrieve all the entries\n"
        "                         without taking into account the parents permissions\n"
        "\n"
                      ...
                      bool read_only)
{
    ...
    if (type) {
        ...
        if (rbh_posix_backend_load_extensions(self, posix, type)) {
            save_errno = errno;
            goto free_root;
        }
