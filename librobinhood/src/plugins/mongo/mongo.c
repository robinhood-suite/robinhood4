...
mongo_backend_init(struct mongo_backend *mongo, const struct rbh_uri *uri)
{
    ...
    if (uri->address) {
         ...
         */
        uint16_t port = (uri->address->port != 0 ? uri->address->port : 27017);

        mongo_uri = mongoc_uri_new_for_host_port(uri->address->host, port);

        if (strcmp(uri->address->username, "") &&
            !mongoc_uri_set_username(mongo_uri, uri->address->username)) {
            mongoc_uri_destroy(mongo_uri);
            errno = EINVAL;
            return -1;
        }

        if (strcmp(uri->address->password, "") &&
            !mongoc_uri_set_password(mongo_uri, uri->address->password)) {
            mongoc_uri_destroy(mongo_uri);
            errno = EINVAL;
            return -1;
        }
