...
static void
regexp(sqlite3_context *ctx, int argc, sqlite3_value **argv)
{
    const char *re, *str;
    pcre_extra *e;
    pcre *p;
...
int
sqlite3_extension_init(sqlite3 *db, char **err, const sqlite3_api_routines *api)
{
    ...
    sqlite3_create_function(db, "REGEXP", 2, SQLITE_UTF8, cache, regexp, NULL,
                            NULL);
    return 0;
}
