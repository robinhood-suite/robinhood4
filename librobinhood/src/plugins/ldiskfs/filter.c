...
fsentry_from_dentry(struct rbh_dentry *dentry, struct rbh_dentry *root, ext2_filsys fs)
{
    ...
    ext2_ino_t ino = dentry->ino;
    struct rbh_value_map inode_xattrs = get_xattrs_from_inode(fs, inode, ino);
...
ldiskfs_iter_new(struct ldiskfs_backend *ldiskfs)
{
    ...
    if (!get_target_type(ldiskfs->fs, iter))
        goto free_iter;

    if(!get_target_index(ldiskfs->fs, iter))
        goto free_iter;
    ...
    if (iter->is_mdt) {
	if(!setup_mdt_iterator(ldiskfs, iter))
	    goto free_iter;
    } else {
	if(!setup_ost_iterator(ldiskfs, iter))
	    goto free_iter;
    }
