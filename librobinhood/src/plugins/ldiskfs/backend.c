                        ...
                        bool read_only)
{
    ...
    if (!thr_str) {
        ...
        ldiskfs->nthreads = strtol(thr_str, &endptr, 10);
        // here strtol can only set errno to EINVAL if the return value is 0
        if( errno == ERANGE || (endptr && *endptr) || (ldiskfs->nthreads < 1)) {
