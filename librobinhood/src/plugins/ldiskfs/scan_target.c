...
static pthread_mutex_t block_iterate_mutex;
static pthread_mutex_t inode_scan_mutex;
               ...
               ext2_dblist dblist)
{
    ...
    // necessary mutex to avoid a race condition in this function
    pthread_mutex_lock(&block_iterate_mutex);
    rc = ext2fs_block_iterate3(ldiskfs->fs, ino, 0, buf, scan_dir_cb, &udata);
    pthread_mutex_unlock(&block_iterate_mutex);
    if (rc) {
        free(buf);
...
scan_inodes(struct ldiskfs_backend *backend)
{
    ...
    pthread_mutex_init(&block_iterate_mutex, NULL);
    pthread_mutex_init(&inode_scan_mutex, NULL);
    ...
    free(threads);
    pthread_mutex_destroy(&block_iterate_mutex);
    pthread_mutex_destroy(&inode_scan_mutex);
