...
err:
    rbh_backend_error_printf(
        "Failed to read extended attributes of inode %d", ino);
    return 0;
}
...
err:
    rbh_backend_error_printf(
            "Could not get file parents from trusted.links extended attribute");
    return default_ret;
}
...
get_hsm_from_xattrs(struct rbh_value_map *xattrs, struct rbh_sstack *sstack)
{
    ...
    if (INODE_XATTR_COUNT - xattrs->count < 3)
        return false;
