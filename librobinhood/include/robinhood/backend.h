...
enum rbh_info {
    ...
    RBH_INFO_FSEVENTS_SOURCE        = 0x00000008U,
    RBH_INFO_LAST_SYNC_START_DATE   = 0x00000010U,
    RBH_INFO_MOUNTPOINT             = 0x00000020U,
    RBH_INFO_SIZE                   = 0x00000040U,
 ...
 *
 * @param backend       a pointer to the backend from where the metadata of the
 *                      file will be retrieved
 * @param path          path to the file to undelete
 * @param fsentry       fsentry containing essential metadata needed to
 *                      'undelete' a given entry
 */
static inline struct rbh_fsentry *
rbh_backend_undelete(struct rbh_backend *backend, const char *path,
                     struct rbh_fsentry *fsentry)
{
 ...
 *
 * This function attempts to create a filter by calling the `build_filter`
 * method of either the main plugin or one of its extensions. The filter is
 * constructed from a string array of two elements: the predicate and its value
 * (e.g., ["-size", "10"]).
 *
 ...
 *             Example: ["-size", "10"].
 *
 * @return     A pointer to the constructed rbh_filter object, or NULL if no
