...
/**
 * Retrieve a fresh entry from the backend.
 *
 * This function fetches the current state of the entry corresponding to
 * \p fsentry from the storage system, ensuring the returned entry reflects
 * the latest data available in the backend.
 *
 * @param  backend  the backend from which to fetch the entry
 * @param  fsentry  the reference entry used to locate the data
 *
 * @return          a newly allocated fsentry representing the current state in
 *                  the filesystem, or NULL on error (with ERRNO set)
 */
struct rbh_fsentry *
