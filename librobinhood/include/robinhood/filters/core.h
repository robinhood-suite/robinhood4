...
import_plugins(struct filters_context *ctx, struct rbh_value_map **info_maps,
               int backend_count);
/**
 * Struct holding information about a backend plugin and its associated extensions,
 * extracted from a backend URI.
 */
struct rbh_backend_plugin_info {
    ...
    const struct rbh_backend_plugin *plugin;
    /**
     * Array of pointers to plugin extensions associated with the backend plugin.
 ...
 * @return A populated rbh_backend_plugin_info struct with plugin and extensions.
 */
struct rbh_backend_plugin_info
